    ...
    if (testResults.basicFunctions && testResults.computationFunctions) {
      ...
      if (basicOptimal && computationOptimal) {
        if (basicOptimal.memoryMB !== computationOptimal.memoryMB) {
          insights.push(
            `Different workloads require different memory configurations: Basic functions perform best at ${basicOptimal.memoryMB}MB, while computation functions perform best at ${computationOptimal.memoryMB}MB`
          );
        } else {
          insights.push(
            `Both basic and computation functions perform optimally at ${basicOptimal.memoryMB}MB`
          );
        }
      }
    }
  ...
  private addFunctionInsights(
    ...
    if (fastest.memoryMB !== baseline.memoryMB) {
      const perfImprovement =
        ((baseline.avgExecutionTime - fastest.avgExecutionTime) / baseline.avgExecutionTime) * 100;
      const costIncrease =
        ((fastest.blendedCostPer1M - baseline.blendedCostPer1M) / baseline.blendedCostPer1M) * 100;
      insights.push(
        `For ${functionType} functions: ${fastest.memoryMB}MB is ${perfImprovement.toFixed(1)}% faster than ${baseline.memoryMB}MB but costs ${costIncrease.toFixed(1)}% more (blended cost)`
      );
    }
    ...
    if (mostCostEfficient.memoryMB !== baseline.memoryMB) {
      const costSavings =
        ((baseline.blendedCostPer1M - mostCostEfficient.blendedCostPer1M) /
          baseline.blendedCostPer1M) *
        100;
      if (costSavings > 0) {
        insights.push(
          `Most cost-efficient ${functionType} config: ${mostCostEfficient.memoryMB}MB saves ${costSavings.toFixed(1)}% blended cost vs 128MB baseline`
        );
      }
    }
  }
