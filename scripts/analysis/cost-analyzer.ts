...
import { TestConfig, FunctionTestResult } from '../types/test-runner';
import { CostAnalysisConfig, OptimalMemoryConfig, CostAnalyzerOutput } from '../types/analysis';
...
class CostAnalyzer {
  ...
  findOptimalMemoryConfig(results: FunctionTestResult[]): OptimalMemoryConfig | null {
    ...
    results.forEach(result => {
      if (result.warmStart) {
        ...
        const totalScore =
          performanceScore * 0.5 + costEfficiencyScore * 0.3 + coldStartBonus * 0.2;
  ...
  analyzeCostEfficiency(results: FunctionTestResult[]): CostAnalyzerOutput | null {
    ...
    results.forEach(result => {
      if (result.warmStart) {
        ...
        const warmCostPer1MInvocations =
          warmGbSeconds * this.config.lambdaPricePerGbSecond * this.config.costCalculationScale;
        ...
        if (result.coldStart && result.coldStart.average > 0) {
          ...
          coldCostPer1MInvocations =
            coldGbSeconds * this.config.lambdaPricePerGbSecond * this.config.costCalculationScale;
          ...
          blendedCostPer1MInvocations =
            coldCostPer1MInvocations * coldPercentage + warmCostPer1MInvocations * warmPercentage;
        }
  ...
  determineBestUseCase(config: CostAnalysisConfig, costAnalysis: CostAnalyzerOutput): string {
    ...
    const warmOptimal = allConfigs.reduce(
      (best: CostAnalysisConfig, current: CostAnalysisConfig) =>
        current.costPer1MInvocations < best.costPer1MInvocations ? current : best
    );
    const perfOptimal = allConfigs.reduce(
      (fastest: CostAnalysisConfig, current: CostAnalysisConfig) =>
    ...
    if (config.memoryMB === warmOptimal.memoryMB) {
      ...
      const costRank = allConfigs.findIndex(
        (c: CostAnalysisConfig) => c.memoryMB === config.memoryMB
      );
