...
export class LambdaTestExecutor {
     ...
     */
    private async makeConcurrentRequests(url: string, count: number): Promise<LambdaFunctionResponse[]> {
        const requests = Array(count).fill(null).map(() => this.makeRequest(url));
        try {
     ...
     */
    public async testFunction(memoryMB: number, config: LambdaTestConfig): Promise<FunctionTestResult> {
