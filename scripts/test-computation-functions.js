...
const COMPUTATION_CONFIGS = [128, 512, 1024, 3008];
 ...
 */
async function testComputationFunction(memoryMB) {
    const url = `${API_BASE_URL}/computation-${memoryMB}`;
    const coldStartResults = [];
    const warmStartResults = [];
    let totalRequests = 0;
    let requestNumber = 1;
    
    console.log(`\nTesting Computation Function ${memoryMB}MB:`);
    ...
    while (coldStartResults.length < TARGET_COLD_STARTS || warmStartResults.length < TARGET_WARM_STARTS) {
        try {
            ...
            responses.forEach((response, index) => {
                if (response.performance && response.executionEnvironment) {
                    ...
                    const result = {
                        ...
                        memorySize: response.executionEnvironment.memorySize,
            ...
            if (totalRequests > 50) { // Lower limit for heavy computation
    ...
    if (allResults.length > 0) {
        ...
        if (coldStartResults.length > 0) {
            const coldExecTimes = coldStartResults.map(r => r.executionTime);
            const coldAvgExec = coldExecTimes.reduce((a, b) => a + b, 0) / coldExecTimes.length;
            const coldMinExec = Math.min(...coldExecTimes);
            const coldMaxExec = Math.max(...coldExecTimes);
            console.log(`    Cold Start  - Avg: ${coldAvgExec.toFixed(2)}ms, Min: ${coldMinExec}ms, Max: ${coldMaxExec}ms`);
        }
        
        // Warm start statistics
        if (warmStartResults.length > 0) {
            const warmExecTimes = warmStartResults.map(r => r.executionTime);
            const warmAvgExec = warmExecTimes.reduce((a, b) => a + b, 0) / warmExecTimes.length;
            const warmMinExec = Math.min(...warmExecTimes);
            const warmMaxExec = Math.max(...warmExecTimes);
            console.log(`    Warm Start  - Avg: ${warmAvgExec.toFixed(2)}ms, Min: ${warmMinExec}ms, Max: ${warmMaxExec}ms`);
        }
...
/**
 * Run all computation function tests
 */
async function runAllTests() {
    console.log('🔥 Starting Computation Function Performance Tests');
    console.log('═'.repeat(60));
    
    const allResults = [];
    
    for (const memoryMB of COMPUTATION_CONFIGS) {
        const result = await testComputationFunction(memoryMB);
        if (result) {
    ...
    // Display summary
    console.log('\n📊 Computation Performance Summary:');
    ...
    allResults.forEach(result => {
        ...
        const coldAvgExec = result.coldStart ? result.coldStart.average.toFixed(0) : 'N/A';
        const warmAvgExec = result.warmStart ? result.warmStart.average.toFixed(0) : 'N/A';
        
        console.log(`${result.memoryMB.toString().padStart(10)} | ${coldCount.toString().padStart(11)} | ${warmCount.toString().padStart(11)} | ${coldAvgExec.padStart(13)} | ${warmAvgExec.padStart(13)}`);
    });
    ...
    console.log('\n✅ Computation function tests completed!');
...
module.exports = { runAllTests, testComputationFunction };
