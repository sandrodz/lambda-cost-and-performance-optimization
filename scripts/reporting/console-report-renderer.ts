...
class ConsoleReportRenderer {
  ...
  private renderFunctionAnalysis(functionType: string, analysisData: FunctionAnalysisData): void {
    ...
    analysisData.warmStart.forEach(config => {
      ...
      console.log(
        `    ${config.memoryMB.toString().padStart(4)}MB | ${config.executionTime.toFixed(timePrecision).padStart(8)}ms | $${config.cost.toFixed(costPrecision).padStart(10)} | ${config.performanceGain >= 0 ? '+' : ''}${config.performanceGain.toFixed(1).padStart(8)}% | ${config.costChange >= 0 ? '+' : ''}${config.costChange.toFixed(1).padStart(9)}%`
      );
    });
  ...
  private renderScenarios(scenarios: ScenarioData): void {
    ...
    if (scenarios.basic) {
      ...
      console.log(
        `    • High Frequency (>100 req/min): ${scenarios.basic.warmOptimal.memoryMB}MB - Best warm start cost`
      );
      console.log(`    • Balanced Workload (10-100 req/min): 1024MB - Good performance/cost ratio`);
      console.log(
        `    • Low Frequency (<10 req/min): ${scenarios.basic.perfOptimal.memoryMB}MB - Minimize cold start impact`
      );
    }
    ...
    if (scenarios.computation) {
      ...
      console.log(
        `    • High Frequency (>100 req/min): ${scenarios.computation.warmOptimal.memoryMB}MB - Best warm start cost`
      );
      console.log(`    • Balanced Workload (10-100 req/min): 1024MB - Good performance/cost ratio`);
      console.log(
        `    • Low Frequency (<10 req/min): ${scenarios.computation.perfOptimal.memoryMB}MB - Minimize cold start impact`
      );
    }
  }
  ...
  private renderDataQuality(dataQuality: DataQualityData): void {
    ...
    if (dataQuality.basic) {
      console.log(
        `  Basic Functions: ${dataQuality.basic.totalConfigurations} memory configurations tested`
      );
      dataQuality.basic.configurations.forEach(config => {
        console.log(
          `    • ${config.memoryMB}MB: ${config.coldCount} cold starts, ${config.warmCount} warm starts`
        );
      });
    }
    ...
    if (dataQuality.computation) {
      console.log(
        `  Computation Functions: ${dataQuality.computation.totalConfigurations} memory configurations tested`
      );
